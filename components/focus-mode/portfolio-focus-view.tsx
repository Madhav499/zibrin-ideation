...
import { Search, ExternalLink, Layers, Cpu, Globe, ArrowUpRight } from "lucide-react";
...
export default function PortfolioFocusView() {
  ...
  const filters = ["All", "AI Engineering", "WebGL & E-Commerce", "Mobile Systems", "Enterprise Software"];
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="p-6 rounded-2xl bg-gradient-to-r from-space-black via-space-black to-space-black border border-cyan-glow/30 backdrop-blur-xl relative overflow-hidden">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-mono transition-all cursor-pointer ${
              selectedFilter === filter
                ...
                : "bg-space-black/70 border border-white/10 text-slate-300 hover:text-white"
            }`}
          >
            {filter}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredProjects.map((proj, idx) => (
          <div
            key={idx}
            className="p-6 rounded-2xl bg-space-black/70 border border-white/10 backdrop-blur-xl hover:border-cyan-glow/40 transition-all duration-300 flex flex-col justify-between group hover:-translate-y-1"
          >
            <div>
              <div className="text-[10px] font-mono text-cyan-glow uppercase tracking-wider mb-2 font-semibold">
                {proj.category}
              </div>
              <h3 className="text-xl font-bold font-syne text-white mb-3 group-hover:text-cyan-glow transition-colors">
                {proj.title}
              </h3>
